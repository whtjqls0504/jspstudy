...
public class Cookie1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  Cookie cookie2 = new Cookie("age", "20");  // 모든 쿠키 값은 String 타입으로 저장한다.
	  ...
	  cookie2.setPath("/servlet/cookie1");   // 서블릿 경로  : request.getRequestURI()
	                                         // cookie3은 경로 설정을 생략했으므로 컨텍스트패스에 저장된다.
	  
	  // 쿠키가 유지되는 시간 설정하기 (생략하면 세션 쿠키가 된다. 브라우저를 닫으면 지워진다.)
	  cookie1.setMaxAge(60 * 60);          // 1시간
	  cookie2.setMaxAge(60 * 60 * 24 * 7); // 7일(일주일)
	                                       // cookie3은 시간 설정을 생략했으므로 세션 쿠키가 된다.
	  
	  // 쿠키 삭제하는 방법은 별도 메소드가 없고,
	  // 쿠키 유지시간을 0으로 설정하고 다시 저장하면 된다.
