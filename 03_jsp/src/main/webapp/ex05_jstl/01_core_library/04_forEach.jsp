...
<body>
	<%--
	 ...
	 <%-- 5~1 출력 --%>
	 <c:forEach var="n" begin="5" end="1" step="-1">
	 	<div>${6-n}</div>
	 </c:forEach>
	 ...
	 %>
	 <c:forEach var="season" items="${seasons}" varStatus="ab">
	 		<div>인덱스 : vs</div>
	 		<div>요소		: </div>
	 </c:forEach>
	...
	<c:forEach var="job" items="${jobs}">
	 ...
	 <%
	 		List<BlogPost> list = Arrays.asList(
	 			...
	 			new BlogPost(2, "두 블로그", 10, LocalDate.of(2023, 1, 2)),
	 			new BlogPost(3, "세 블로그", 10, LocalDate.of(2023, 1, 2))
	 	);
	 		pageContext.setAttribute("list", list);
	 %>
	 
	 <c:forEach var="blogPost" items="${Post" items="${list}">
	 		<div class="blog">
	 			<span class="blogPostNo">${blogPost.blogPostNo}</span>
	 			<span class="title"><a href=">${blogPost.title}</span>
