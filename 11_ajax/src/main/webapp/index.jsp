  ...
  $.ajax({
    ...
    url: '${contextPath}/member/list.do',
  	...
  	success : function(resData) {
			...
			// obj.memberList		/// 목록 꺼내기?
			$('#member_count').text(obj.memberCount);		/// 갯수 꺼내기?
  ...
  $('#email').keyup(function () {
      ...
      success : function(resData) {		// resData === '{"ableEmail" : true}'
        ...
        ableEmail = obj.ableEmail;
      	if(ableEmail){
      	  ...
      	  $('#msg_email').text('사용할 수 없는 이메일입니다. ');
        }
      }
    })
  })
	 ...
	 $(document).on('click', '.btn_detail', function () {
	   ...
	   $.ajax({
	     ...
	     success : function(resData) {						// resData === '{}'
        	var obj = JSON.parse(resData);				// {"member" : {"memberNo": 1, ... }}
    ...
    $('#btn_modify').click(function() {
      $.ajax({
        ...
        data : $('#frm_member').serialize(), // 전달할 값이 많다면 form을 사용하라.
        dataType : 'text',
